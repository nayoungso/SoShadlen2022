    ...
    for i = 1:length(filename)
        ...
        for j = 1:length(trials)
            ...
            if (trials(j).taskid == taskid)&&(trials(j).response >=0)&&(trials(j).mean_maxFR >= 10)
                ...
                if trials(j).response ==1   % correct trials only for the final choice saccade epoch
...
for i = 1:length(coh_set) % to calculate residual FR for each condition (baseline adjustment)
...
maxy1 = max([DotsOn{12} ChoiceSacc{12} Slib{12} DotsOn2{12}]);
maxy1 = maxy1+5;
