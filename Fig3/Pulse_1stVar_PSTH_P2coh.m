function Pulse_1stVar_PSTH_P2coh
...
pre_dots2 = -150; post_dots2 = 400;  % for 2nd dots 
...
for p=1:4
    ...
    if p==1
        taskid = 30; filename = filename_a; filepath = [path,'/A/r/'];
    elseif p==2
        taskid = 30; filename = filename_b; filepath = [path,'/A/g/'];
    elseif p==3
        taskid = 31; filename = filename_c; filepath = [path,'/B/r/'];
    elseif p==4
        taskid = 31; filename = filename_d; filepath = [path,'/B/g/'];
    end
    ...
    for i = 1:length(filename)
        ...
        for j = 1:length(trials)
            ...
            if (trials(j).taskid == taskid)&&(trials(j).response>=0)&&(trials(j).mean_maxFR >= 10)
                ...
                coh = trials(j).dot_coh(:)*(1-2*dir_sign);  % signed coh 
...
xlim([-100 post_dots2]);
set(gca, 'TickDir','out');
ylabel('Firing rates, sorted by coh1')
title('aligned on P2')
...
xlim([-100 post_dots2]);
set(gca, 'TickDir','out');
ylabel('Firing rates, sorted by coh2')
title('aligned on P2')
...
xlim([-100 post_dots2]);
set(gca, 'TickDir','out');
ylabel('Detrended firing rates, sorted by coh2')
title('aligned on P2')
